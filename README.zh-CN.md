...
- `.github/workflows/ci.yml` 只负责验证 Pull Request、`main` 与手动运行，不授予容器仓库写权限。它执行后端测试、vet、可达 Go 漏洞检查、Swagger 漂移检查、前端审计/lint/build，以及 `push: false` 的 amd64/arm64 容器构建。
- 仅推送标签不会发布镜像。`.github/workflows/release-image.yml` 只在 GitHub Release 正式发布后运行；标签必须是严格的 `vMAJOR.MINOR.PATCH`（可带 SemVer 预发布后缀），必须与 `VERSION` 完全一致，且对应提交必须包含在 `main` 中。
- GHCR 写入隔离在受保护的 `release` environment 后。多架构镜像按 digest 汇总，附带 SBOM 与 provenance attestation，并通过 manifest 检查和 `/healthz` 冒烟测试。
- 正式版发布精确的 `vX.Y.Z`、`X.Y.Z`、`X.Y`、`X` 和 `latest`；预发布版只发布精确版本标签，不公开架构临时标签。
...
正式发布 GitHub Release 后才会创建 `linux/amd64` 与 `linux/arm64` 的 GHCR 镜像；仅推送标签不会发布。在本仓库发布首个 Release 前，Docker Compose 默认从当前源码进行本地构建。
