...
func (h *Handler) sync(c *gin.Context) {
	...
	for {
		...
		case value := <-result:
			if value.err != nil {
				var partial *modelapp.PartialSyncError
				if errors.As(value.err, &partial) {
					_ = writeModelSyncEvent(c, "complete", gin.H{
						"synced": value.synced, "partial": true,
						"accountsSucceeded": partial.Succeeded, "accountsFailed": partial.Failed,
					})
					return
				}
				code, message := "modelSyncFailed", "同步模型能力失败"
				if errors.Is(value.err, modelapp.ErrSyncInProgress) {
					code, message = "modelSyncInProgress", "模型同步正在进行"
				}
				_ = writeModelSyncEvent(c, "error", gin.H{"code": code, "message": message})
				return
			}
