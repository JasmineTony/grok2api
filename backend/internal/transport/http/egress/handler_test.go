...
func TestNodeRequestInputPreservesAccountCapacity(t *testing.T) {
	capacity := 37
	input := (nodeRequest{AccountCapacity: &capacity}).input()
	if input.AccountCapacity == nil || *input.AccountCapacity != capacity {
		t.Fatalf("account capacity = %v, want %d", input.AccountCapacity, capacity)
	}
}
