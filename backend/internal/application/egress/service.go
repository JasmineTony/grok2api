...
func NewService(repository repository.EgressRepository, cipher *security.Cipher, browserUA string, accounts ...AccountBindingRepository) *Service {
	...
	service := &Service{repository: repository, cipher: cipher, browserUA: strings.TrimSpace(browserUA), now: func() time.Time { return time.Now().UTC() }, dial: dialer.DialContext}
