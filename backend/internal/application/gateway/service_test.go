...
func TestGatewayFailsOverBeforeReturningBody(t *testing.T) {
	...
	result, err := service.CreateResponse(ctx, Input{RequestID: "req-1", ClientKey: clientKey, PublicModel: "grok-test", Body: []byte(`{"model":"grok-test"}`), PromptCacheSeed: "claude-session", GrokTurnIndex: "3", ForcedProvider: string(account.ProviderBuild)})
	if err != nil {
		t.Fatal(err)
	}
