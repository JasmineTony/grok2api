...
func (s *Service) GenerateImage(ctx context.Context, input ImageGenerationInput) (*Result, error) {
	...
	}, input.Streaming, input.Resolution, input.Quality, input.Count, 0, input.ForcedProvider)
}
...
func (s *Service) EditImage(ctx context.Context, input ImageEditInput) (*Result, error) {
	...
	}, input.Streaming, input.Resolution, input.Quality, input.Count, len(input.ImageURLs), input.ForcedProvider)
}
