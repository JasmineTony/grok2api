...
type videoAssetStore interface {
	...
	InspectVideo(ctx context.Context, id string) (mediadomain.Asset, error)
	OpenVideo(ctx context.Context, id string) (mediadomain.Asset, io.ReadCloser, error)
	InspectInputAsset(ctx context.Context, id string) (mediadomain.Asset, error)
	...
	if routeErr == nil && ownership == nil && input.ForcedEgressNodeID == 0 {
		for _, candidate := range orderedRoutes {
			...
			if candidate.Provider == accountdomain.ProviderBuild {
				...
				candidateAffinityKey = identity.affinityKey
			}
	...
	handleResponse:
		...
		afterTeamRateLimit:
			...
			if lease.QuotaMode != "" && response.StatusCode == http.StatusTooManyRequests {
				...
				s.selector.MarkFreeQuotaExhausted(ctx, credential, 0, 0, quotaRecoveryHints{Billing: lease.Billing, QuotaMode: lease.QuotaMode, RetryAfter: retryAfter})
				failureHandled = true
			} else if lastFailure.QuotaExhausted {
				s.selector.MarkPaymentQuotaExhausted(ctx, credential, quotaRecoveryHints{Billing: lease.Billing, QuotaMode: lease.QuotaMode, RetryAfter: retryAfter})
				failureHandled = true
			}
			if lastFailure.AccountBlocked {
				...
				if credential.Provider == accountdomain.ProviderBuild {
					...
					if s.markBuildChatDeniedAsReauth.Load() {
						failureHandled = s.markReauthRequired(ctx, input.RequestID, credential, fmt.Sprintf("%s chat endpoint access denied", credential.Provider)) || failureHandled
					}
				} else {
...
// shouldStopForNonAccountFingerprint stops retries after repeated identical non-account failures.
func shouldStopForNonAccountFingerprint(fingerprints map[string]int, failure *UpstreamFailure) bool {
	...
	if failure.HTTPStatus == http.StatusForbidden || failure.Fingerprint == "429:team_model_rate_limit" {
		return false
	}
...
func isRetryableResponse(response *provider.Response, upstreamProviders ...accountdomain.Provider) bool {
	if response == nil || !isRetryable(response.StatusCode) {
		return false
	}
	if len(upstreamProviders) > 0 && forcesAccountFailover(response.StatusCode, upstreamProviders[0]) {
		return true
	}
