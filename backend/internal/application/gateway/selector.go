...
func (s *Selector) MarkFreeQuotaExhausted(ctx context.Context, credential account.Credential, used, limit int64, hintValues ...quotaRecoveryHints) {
	hints := quotaRecoveryHints{}
	if len(hintValues) > 0 {
		hints = hintValues[0]
	}
	now := time.Now().UTC()
