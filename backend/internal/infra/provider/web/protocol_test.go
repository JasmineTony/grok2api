...
func TestBuildDirectFileUploadBodySanitizesUnsafeFilename(t *testing.T) {
	...
	// Assert the raw Content-Disposition instead of part.FileName(): the latter applies
	// filepath.Base, which treats the escaped backslash as a separator on Windows.
	disposition := part.Header.Get("Content-Disposition")
	if disposition != `form-data; name="file"; filename="a\\b\"c_.png"` {
...
func TestModelsUseLowestSufficientTierFirst(t *testing.T) {
	...
	if got := adapter.TierOrderForQuotaMode("grok-imagine-video", account.QuotaModeWebVideo720p); !slices.Equal(got, []account.WebTier{account.WebTierBasic, account.WebTierSuper, account.WebTierHeavy}) {
		t.Fatalf("720p video tier order = %v", got)
	}
	if got := adapter.TierOrderForQuotaMode("grok-imagine-video", account.QuotaModeWebVideo); !slices.Equal(got, []account.WebTier{account.WebTierSuper, account.WebTierHeavy}) {
		t.Fatalf("non-720p video tier order = %v", got)
	}
}
