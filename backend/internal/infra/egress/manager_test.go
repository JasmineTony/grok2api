...
func TestAcquireCredentialUsesBoundEgressNode(t *testing.T) {
	...
	nodeID := uint64(2)
	encryptedProxy, err := cipher.Encrypt("http://account-bound.example:8080")
	if err != nil {
		t.Fatal(err)
	}
	repositoryValue := &policyEgressRepository{
		egressRepositoryTestStub: egressRepositoryTestStub{nodes: []domain.Node{
			...
			{ID: nodeID, Name: "account-bound", Scope: domain.ScopeWeb, Enabled: true, Health: 1, EncryptedProxyURL: encryptedProxy},
		}},
