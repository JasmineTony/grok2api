...
func (m *Manager) AcquireCredential(ctx context.Context, scope domain.Scope, credential accountdomain.Credential) (*Lease, error) {
	...
	boundNodeID := credential.EgressNodeID
	var legacyPolicy *domain.AccountPolicy
	...
	if legacyPolicy != nil && legacyPolicy.Strategy == domain.AccountPolicyDirect {
		lease, _, err := m.leaseForNode(ctx, scope, strconv.FormatUint(credential.ID, 10), credential.EncryptedCloudflareCookie, isGrokWebScope(scope) && m.clearanceMode() == "flaresolverr", domain.Node{ID: 0, Name: "direct", Scope: scope, Enabled: true, Health: 1})
		return lease, err
	}
	if legacyPolicy != nil && legacyPolicy.Strategy == domain.AccountPolicyNode && legacyPolicy.EgressNodeID != nil {
		boundNodeID = *legacyPolicy.EgressNodeID
	}
	ctx = WithEgressNode(ctx, boundNodeID)
	lease, _, err := m.acquire(ctx, scope, strconv.FormatUint(credential.ID, 10), true, credential.EncryptedCloudflareCookie, boundNodeID)
	if err != nil && legacyPolicy != nil && legacyPolicy.Strategy == domain.AccountPolicyNode && legacyPolicy.AllowDirectFallback {
		directLease, _, directErr := m.leaseForNode(ctx, scope, strconv.FormatUint(credential.ID, 10), credential.EncryptedCloudflareCookie, isGrokWebScope(scope) && m.clearanceMode() == "flaresolverr", domain.Node{ID: 0, Name: "direct", Scope: scope, Enabled: true, Health: 1})
		return directLease, directErr
	}
	return lease, err
}
...
func (m *Manager) acquire(ctx context.Context, scope domain.Scope, affinity string, allowDirect bool, encryptedCredentialCookies string, boundNodeID uint64) (*Lease, bool, error) {
	...
	if boundNodeID != 0 {
		...
		fallbackUnavailable := func(primaryErr error) (*Lease, bool, error) {
			if qualityProbe {
		...
		for {
			...
			if err != nil {
				...
				return fallbackUnavailable(primaryErr)
			}
			if !domain.SupportsScope(selected.Scope, scope) {
				return fallbackUnavailable(fmt.Errorf("绑定出口节点 %d 与 %s 作用域不兼容", boundNodeID, scope))
			}
			if !selected.Enabled && !qualityProbe {
				return fallbackUnavailable(fmt.Errorf("绑定出口节点 %d 已禁用", boundNodeID))
			}
			if strings.TrimSpace(selected.EncryptedProxyURL) == "" {
				return fallbackUnavailable(fmt.Errorf("绑定出口节点 %d 未配置代理地址", boundNodeID))
			}
			...
			if !qualityProbe && !proxyPool && selected.CooldownUntil != nil && now.Before(*selected.CooldownUntil) {
				...
				return fallbackUnavailable(fmt.Errorf("绑定出口节点 %d 正在冷却", boundNodeID))
			}
...
func (m *Manager) clientFor(id uint64, scope domain.Scope, proxyURL, userAgent, cookies string, sticky bool, accountIdentity ...string) (cachedClient, error) {
	identity := ""
	if len(accountIdentity) > 0 {
		identity = accountIdentity[0]
	}
	return m.clientForWithOptions(id, scope, proxyURL, userAgent, cookies, sticky, identity, clientOptions{})
}
