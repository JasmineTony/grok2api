...
var schemaModels = []any{
	...
	&notificationModel{},
	&requestPolicyModel{},
	&requestSnapshotModel{},
	&usageRollupCheckpointModel{},
...
var schemaIndexes = []string{
	...
	"CREATE INDEX IF NOT EXISTS idx_notifications_dedup ON notifications(dedup_key, created_at DESC, id DESC)",
	"CREATE INDEX IF NOT EXISTS idx_notifications_expiry ON notifications(expires_at, id)",
	"CREATE INDEX IF NOT EXISTS idx_request_policies_order ON request_policies(enabled, priority ASC, id ASC)",
	"CREATE INDEX IF NOT EXISTS idx_request_snapshots_expiry ON request_snapshots(expires_at, id)",
	"CREATE UNIQUE INDEX IF NOT EXISTS uidx_usage_rollups_dimensions ON usage_rollups(bucket_kind, bucket_start, provider, model, account_id, client_key_id)",
