...
func TestDashboardRepositorySnapshot(t *testing.T) {
	...
	audits := []requestAuditModel{
		{RequestID: "success-1", ClientKeyID: 1, AccountID: &active.ID, ModelRouteID: 1, ModelPublicID: "grok-primary", Provider: "grok_build", Operation: "responses", UsageSource: "upstream", StatusCode: 200, Streaming: true, OutputTokens: 90, TotalTokens: 100, FirstTokenMS: &firstTokenOne, DurationMS: 1100, CreatedAt: now.Add(-23 * time.Hour)},
		{RequestID: "success-2", ClientKeyID: 1, AccountID: &active.ID, ModelRouteID: 1, ModelPublicID: "grok-secondary", Provider: "grok_web", Operation: "responses", UsageSource: "upstream", StatusCode: 201, Streaming: true, OutputTokens: 10, TotalTokens: 50, FirstTokenMS: &firstTokenTwo, DurationMS: 1300, CreatedAt: now.Add(-time.Hour)},
		{RequestID: "failed", ClientKeyID: 1, AccountID: &active.ID, ModelRouteID: 1, ModelPublicID: "grok-primary", Provider: "grok_build", Operation: "responses", UsageSource: "upstream", StatusCode: 500, Streaming: true, OutputTokens: 100, TotalTokens: 10, DurationMS: 550, CreatedAt: now.Add(-2 * time.Hour)},
...
func TestDashboardRepositoryCounts2xxWithErrorAsFailure(t *testing.T) {
	...
	if rolledSnapshot.Usage.Requests != 2 || rolledSnapshot.Usage.SuccessfulRequests != 1 || rolledSnapshot.Usage.FailedRequests != 1 {
		t.Fatalf("rolled usage = %#v", rolledSnapshot.Usage)
	}
}
...
func BenchmarkDashboardUsageAggregate(b *testing.B) {
	...
	b.Run("performance", func(b *testing.B) {
		for b.Loop() {
			var usage dashboardBenchmarkUsage
			if err := database.db.WithContext(ctx).Model(&requestAuditModel{}).Select(dashboardMetricSelect(false)).Where("created_at >= ? AND created_at < ?", start, now.Add(time.Second)).Scan(&usage).Error; err != nil {
				b.Fatal(err)
			}
		}
	})
