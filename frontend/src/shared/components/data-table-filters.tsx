    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start" className="w-52">
        {filters.map((filter) => {
          ...
          const selectedLabel =
            filter.selectedLabel ?? findSelectedLabel(filter.options, filter.value);
          return (
            <DropdownMenuSub key={filter.id}>
              <DropdownMenuSubTrigger>
                <span>{filter.label}</span>
                  ...
                  {filter.options.map((option) => {
                    ...
                    const narrowedLabel =
                      option.value === filter.value
                        ? null
                        : findGroupLabel(option.groups, filter.value);
                    return (
                      <DropdownMenuSub
                        key={option.value}
                        {...(option.onGroupsOpenChange === undefined
                          ? {}
                      ...
                      >
                        ...
                        >
                          {option.groupSearch ? (
                            <div className="sticky top-0 z-20 border-b bg-popover/95 px-2 py-1.5 backdrop-blur-sm">
                              <div className="relative">
                                ...
                                <Input
                                  ...
                                  onChange={(event) =>
                                    option.groupSearch?.onChange(event.target.value)
                                  }
                                  onKeyDown={(event) => event.stopPropagation()}
                                />
                              </div>
