...
export function VirtualTableBody<T>({
  ...
  const updateRange = useCallback(() => {
    if (!enabled || !bodyRef.current) {
      setRange((current) =>
        current.start === 0 && current.end === items.length
          ? current
    ...
    const scrollContainer = bodyRef.current.closest<HTMLElement>(
    ...
    const end = Math.min(
      items.length,
      Math.ceil(Math.max(relativeTop, relativeBottom) / rowHeight) + overscan,
    );
    setRange((current) =>
  ...
  useLayoutEffect(() => {
    const scrollContainer =
      bodyRef.current?.closest<HTMLElement>("[data-slot=table-scroll-container]") ?? null;
  ...
  const safeEnd =
    enabled && items.length > 0 ? Math.max(safeStart + 1, Math.min(items.length, range.end)) : 0;
