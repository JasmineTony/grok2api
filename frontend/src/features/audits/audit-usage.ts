...
export type AuditUsageView = {
  ...
  mediaItems?: AuditUsageItem[] | undefined;
  tokenItems?: AuditUsageItem[] | undefined;
...
): AuditUsageItem[] | undefined {
  if (audit.operation === "video") {
    return [
      {
        key: "mediaInput",
        label: labels.mediaInput,
        value: labels.imageCount(audit.mediaInputImages),
      },
      {
        key: "mediaOutput",
        label: labels.mediaOutput,
        value: labels.secondsCount(audit.mediaOutputSeconds),
      },
    ];
  }
  if (
    ...
    return [
      ...
      },
      {
        key: "mediaOutput",
        label: labels.mediaOutput,
        value: labels.imageCount(audit.mediaOutputImages),
      },
    ];
  }
  return undefined;
  ...
  return [
    {
      key: "input",
      label: labels.input,
      value: formatAuditTokenValue(audit.inputTokens, available, formatNumber),
    },
    {
      key: "output",
      label: labels.output,
      value: formatAuditTokenValue(audit.outputTokens, available, formatNumber),
    },
    {
      key: "cached",
      label: labels.cached,
