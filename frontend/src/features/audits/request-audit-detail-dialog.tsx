...
export function RequestAuditDetailDialog({
  ...
  const detailQuery = useQuery({
    ...
    queryFn: () => getRequestAudit(apiClient, audit?.id ?? ""),
    enabled: open && audit !== null,
  });

  const attempts = detailQuery.data?.attempts ?? [];
  const selectedAttempt =
    attempts.find((attempt) => attempt.number === selectedNumber) ?? attempts[0];
      ...
      <DialogContent className="flex h-[min(620px,calc(100svh-2rem))] max-h-[calc(100svh-2rem)] min-h-0 flex-col gap-0 overflow-hidden p-0 text-xs sm:max-w-4xl">
        <DialogHeader className="shrink-0 px-5 py-4 pr-12">
          ...
          <DialogDescription className="flex min-w-0 flex-wrap gap-x-4 gap-y-0.5">
            <span className="truncate" title={audit?.requestId}>
              {audit?.requestId}
            </span>
          ...
          attempts.length > 0 && selectedAttempt ? (
            <div className="grid min-h-0 flex-1 grid-rows-[auto_minmax(0,1fr)] lg:grid-cols-[190px_minmax(0,1fr)] lg:grid-rows-1">
              <aside className="flex min-h-0 min-w-0 flex-col overflow-hidden bg-muted/25 p-2.5">
                <p className="mb-1 shrink-0 px-2 text-muted-foreground">
                  {t("audits.attemptTimeline")}
                </p>
      ...
      <span className="flex items-center justify-between gap-2">
        ...
        {attempt.upstreamStatusCode ? (
          <StatusBadge
            ...
            failed={attempt.stage === "response_stream"}
          />
        ) : null}
      </span>
    </button>
      ...
      <Tabs defaultValue="overview" className="min-h-0 flex-1 overflow-hidden px-4 pb-4 sm:px-5">
        ...
        <TabsContent value="body" className="min-h-0 flex-1 overflow-hidden pt-3">
          <CodePanel
            ...
            displayValue={formattedResponseBody(attempt)}
...
function AttemptOverview({ attempt }: { attempt: AuditAttemptDTO }) {
  ...
  return (
    <div className="grid gap-x-10 gap-y-6 px-1 py-4 sm:grid-cols-2">
      <OverviewField
        label={t("audits.attemptStartedAt")}
        value={formatDateTime(attempt.startedAt, i18n.language)}
      />
      <OverviewField
        label={t("audits.duration")}
        value={`${formatNumber(attempt.durationMs, i18n.language)} ms`}
      />
      <OverviewField
      ...
      <OverviewField
        ...
        value={
          ...
          (attempt.upstreamStatusCode ? String(attempt.upstreamStatusCode) : "-")
        }
      />
      <OverviewField
      ...
      {attempt.transportError ? (
        <OverviewField
          ...
          label={
            attempt.source === "gateway_transport"
              ? t("audits.transportError")
              : t("audits.attemptError")
          }
  ...
  return (
    <div className={cn("flex min-w-0 items-start gap-4", className)}>
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-1 break-all" title={value}>
          {value}
        </p>
      </div>
      {copy ? (
...
function CodePanel({
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden rounded-md bg-muted/25">
      ...
      <div className="min-h-0 flex-1 overflow-auto whitespace-pre-wrap break-words p-3">
        {displayValue}
      </div>
    </div>
  );
...
function HeadersPanel({
  ...
  if (entries.length === 0)
    return <EmptyPanel icon={<Braces />} message={t("audits.emptyResponseHeaders")} />;
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden rounded-md bg-muted/15">
      ...
      <div className="min-h-0 flex-1 space-y-1 overflow-auto px-2 pb-2">
        {entries.map(([name, values]) => (
          <div
            key={name}
            className="grid gap-1 rounded-md px-2 py-2 hover:bg-background/60 sm:grid-cols-[180px_minmax(0,1fr)] sm:gap-4"
          >
            <span className="break-all text-muted-foreground">{name}</span>
            <div className="min-w-0 space-y-1">
              {values.map((value, index) => (
                <span key={`${name}-${index}`} className="block break-all">
                  {value}
                </span>
              ))}
            </div>
...
function ErrorChainPanel({ attempt, copyValue }: { attempt: AuditAttemptDTO; copyValue: string }) {
  const { t } = useTranslation();
  if (attempt.errorChain.length === 0)
    return <EmptyPanel icon={<Network />} message={t("audits.emptyErrorChain")} />;
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden rounded-md bg-muted/15">
      <div className="flex h-10 shrink-0 items-center justify-between px-3">
        <span className="text-muted-foreground">
        ...
        {attempt.errorChain.map((frame, index) => (
          <li key={`${frame.type}-${index}`}>
            <div className="flex items-center gap-2 text-muted-foreground">
              <span>#{index + 1}</span>
              <span className="break-all">{frame.type}</span>
            </div>
...
function formattedResponseBody(attempt: AuditAttemptDTO): string {
  ...
  const contentType =
    Object.entries(attempt.responseHeaders)
      ...
      .join(";") ?? "";
  if (attempt.stage !== "response_stream" && !contentType.toLowerCase().includes("json"))
...
function StatusBadge({ statusCode, failed = false }: { statusCode: number; failed?: boolean }) {
  ...
  return (
    <Badge variant="secondary" className={cn("min-w-9 justify-center px-1.5", className)}>
      {statusCode}
    </Badge>
  );
}
