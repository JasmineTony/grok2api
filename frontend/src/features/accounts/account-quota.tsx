    ...
    <div className="w-full min-w-0 space-y-1.5">
      <div className="flex items-start justify-between gap-3 text-[11px] font-normal">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  ...
                  aria-label={t("accounts.freeEstimatedDescription")}
                >
        ...
        <span className="shrink-0 text-muted-foreground">
          {isEstimated ? "≈" : ""}
          {formatNumber(quota.usagePercent, locale, 1)}%
        </span>
      </div>
...
function BuildQuota({
  ...
  const weeklyPercent = Math.max(
    0,
    Math.min(100, percentageQuota ? quota.usagePercent : (billing?.creditUsagePercent ?? 0)),
  );
  ...
  return (
    <div className="w-full min-w-0 space-y-1.5">
      <div
        className={cn(
          "grid w-full min-w-0 divide-x divide-border/70",
          hasWeekly && hasMonthly ? "grid-cols-2" : "grid-cols-1",
        )}
      >
        {hasWeekly ? (
          <Tooltip>
            <TooltipTrigger asChild>
              <button
                type="button"
                className="min-w-0 px-2 text-left font-normal first:pl-0 last:pr-0"
              >
                <div className="flex items-center justify-between gap-1 text-[11px]">
                  <span className="truncate text-muted-foreground">
                    {t("accounts.weeklyQuota")}
                  </span>
                  <span className="shrink-0 tabular-nums">
                    {formatNumber(weeklyPercent, locale, 1)}%
                  </span>
                </div>
                <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-muted">
                  <div className="h-full bg-primary" style={{ width: `${weeklyPercent}%` }} />
                </div>
              </button>
            </TooltipTrigger>
            <TooltipContent>
              <div>
                {t("accounts.weeklyLimit", {
                  percent: formatNumber(100 - weeklyPercent, locale, 1),
                })}
              </div>
              <div className="text-muted-foreground">
                {weeklyPeriodEnd
                  ? t("accounts.quotaResetAt", { time: formatDateTime(weeklyPeriodEnd, locale) })
                  : t("accounts.quotaResetUnknown")}
              </div>
            </TooltipContent>
          </Tooltip>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              ...
              >
                <div className="flex items-center justify-between gap-1 text-[11px]">
                  <span className="truncate text-muted-foreground">
                    {t("accounts.monthlyQuota")}
                  </span>
                  <span className="shrink-0 tabular-nums">
                    {formatNumber(quota.used, locale, 2)}/{formatNumber(quota.limit, locale, 2)}
                  </span>
                </div>
                <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-muted">
                  <div className="h-full bg-primary" style={{ width: `${monthlyPercent}%` }} />
                </div>
              </button>
            </TooltipTrigger>
            <TooltipContent>
              <div>
                {t("accounts.paidQuotaDetails", {
                  remaining: formatNumber(quota.remaining, locale, 2),
                })}
              </div>
              <div className="text-muted-foreground">
                {billing?.billingPeriodEnd
                  ? t("accounts.quotaResetAt", {
                      time: formatDateTime(billing.billingPeriodEnd, locale),
                    })
                  : t("accounts.quotaResetUnknown")}
              </div>
            </TooltipContent>
...
export function ConsoleQuota({
  windows,
  locale,
}: {
  windows: NonNullable<AccountDTO["quotaWindows"]>;
  locale: string;
}) {
  const { t } = useTranslation();
  if (windows.length === 0)
        ...
        return (
          <Tooltip key={mode}>
            <TooltipTrigger asChild>
              <button
                type="button"
                className="block w-full min-w-0 px-2 text-left first:pl-0 last:pr-0"
              >
                <div className="flex items-center justify-between gap-1 text-[11px]">
                  <span
                    className={cn(
                      "truncate",
                      exhausted ? "text-amber-600 dark:text-amber-400" : "text-muted-foreground",
                    )}
                  >
                    {label}
                  </span>
                  {exhausted ? (
                    <span className="shrink-0 truncate text-amber-600 dark:text-amber-400">
                      {t("accounts.imagineQuotaExhausted")}
                    </span>
                  ) : hasTotal ? (
                    <span className="shrink-0 tabular-nums text-muted-foreground">
                      {formatNumber(window.remaining, locale, 0)}/
                      {formatNumber(window.total, locale, 0)}
                    </span>
                  ) : (
                    <span className="shrink-0 tabular-nums text-muted-foreground">
                      {t("accounts.imagineQuotaRemaining", {
                        remaining: formatNumber(window.remaining, locale, 0),
                      })}
                    </span>
                  )}
                </div>
...
function WeeklyWebQuota({
  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <button type="button" className="block w-full min-w-0 text-left">
          <div className="flex items-center justify-between gap-2 text-[11px]">
            {breakdown.length > 0 ? (
              <div className="flex min-w-0 items-center gap-2.5 overflow-hidden text-muted-foreground">
                {breakdown.slice(0, 3).map((item) => (
                  <span key={item.productCode} className="flex shrink-0 items-center gap-1">
                    <span
                      className={cn("size-1.5 rounded-full", quotaProductColor(item.productCode))}
                    />
                    <span>{quotaProductLabel(item.productCode, t)}</span>
                    <span className="tabular-nums text-foreground">
                      {formatNumber(item.usagePercent, locale, 1)}%
                    </span>
                  </span>
                ))}
                {breakdown.length > 3 ? (
                  <span className="shrink-0">+{breakdown.length - 3}</span>
                ) : null}
              </div>
            ) : (
            ...
            {breakdown.length > 0 ? (
              breakdown.map((item) => (
                <div
                  ...
                  className={cn("h-full shrink-0", quotaProductColor(item.productCode))}
                  style={{ width: `${Math.max(0, Math.min(100, item.usagePercent))}%` }}
                />
              ))
            ) : (
      ...
      <TooltipContent>
        <div>
          {t("accounts.webWeeklyQuotaUsage", {
            remaining: formatNumber(100 - usedPercent, locale, 1),
          })}
        </div>
        <div className="text-muted-foreground">
          {window.resetAt
            ...
            {breakdown.map((item) => (
              <div key={item.productCode} className="flex items-center justify-between gap-4">
                <span className="flex items-center gap-1.5">
                  <span
                    className={cn("size-2 rounded-full", quotaProductColor(item.productCode))}
                  />
                  {quotaProductLabel(item.productCode, t)}
                </span>
  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <button
          type="button"
          className={cn("block w-full min-w-0 text-left", compact && "px-2 first:pl-0 last:pr-0")}
        >
          <div className="flex items-center justify-between gap-1 text-[11px]">
            <span className="truncate text-muted-foreground">{mode}</span>
            <span className="shrink-0 tabular-nums">
              {formatNumber(used, locale, 0)}/{formatNumber(window.total, locale, 0)}
            </span>
          </div>
          <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-muted">
            <div className="h-full bg-primary" style={{ width: `${percent}%` }} />
      ...
      <TooltipContent>
        <div>
          {t("accounts.webModeQuotaRemaining", {
            mode,
            remaining: formatNumber(window.remaining, locale, 0),
          })}
        </div>
...
function quotaProductColor(code: number): string {
  const colors: Record<number, string> = {
    0: "bg-quota-product-0",
    1: "bg-quota-product-1",
    2: "bg-quota-product-2",
    3: "bg-quota-product-3",
    4: "bg-quota-product-4",
    5: "bg-quota-product-5",
    6: "bg-quota-product-6",
  };
