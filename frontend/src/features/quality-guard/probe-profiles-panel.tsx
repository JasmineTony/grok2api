...
export function ProbeProfilesPanel() {
  ...
  const query = useQuery({
    queryKey: ["quality-guard-profiles"],
    queryFn: () => listProbeProfiles(apiClient),
  });
  const invalidate = () => {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (editing && !editing.built_in) {
        return updateProbeProfile(apiClient, editing.id, payload);
      }
      return createProbeProfile(apiClient, payload);
    },
    onSuccess: () => {
  ...
  const activateMutation = useMutation({
    mutationFn: (profile: ProbeProfile) =>
      updateProbeProfile(apiClient, profile.id, {
        name: profile.name,
  ...
  const deleteMutation = useMutation({
    mutationFn: (profile: ProbeProfile) => deleteProbeProfile(apiClient, profile.id),
    onSuccess: () => {
  ...
  return (
    <section className="overflow-hidden rounded-lg bg-card">
              ...
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <Badge variant={profile.built_in ? "secondary" : "outline"}>
                    {t(
                      profile.built_in
                        ? "qualityGuard.profileBuiltin"
                        : "qualityGuard.profileCustom",
                    )}
                  </Badge>
      ...
      <Dialog
        ...
        onOpenChange={(open) => {
          if (!open && !saveMutation.isPending) setEditing(undefined);
        }}
      >
          ...
          >
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value="last_line">
                    {t("qualityGuard.profileMatchLastLine")}
                  </SelectItem>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                {t("qualityGuard.profileExpectedHelp")}
              </p>
            </div>
            <div className="space-y-2">
            ...
            <div className="flex items-start justify-between gap-4 rounded-md border p-3">
              <div className="space-y-1">
                <Label htmlFor="probe-profile-thinking">
                  {t("qualityGuard.profileRequireThinking")}
                </Label>
                <p className="text-xs text-muted-foreground">
                  {t("qualityGuard.profileRequireThinkingHelp")}
                </p>
              </div>
              <Switch
            ...
            <DialogFooter>
              ...
              {readonly ? null : (
                <Button
                  type="submit"
                  size="sm"
                  disabled={!draft.name.trim() || !draft.prompt.trim() || saveMutation.isPending}
                >
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t("qualityGuard.profileDeleteConfirm", { name: deleting?.name ?? "" })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
