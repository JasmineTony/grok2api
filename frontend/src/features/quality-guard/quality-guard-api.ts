...
const nodeStateValidator = hasShape({
  active_soft_strikes: isNumber,
  passive_soft_strikes: isNumber,
  error_strikes: isNumber,
  quarantined_until: isNumber,
  disabled_by_guard: isBoolean,
  last_reason: isString,
  last_probe_at: isNumber,
  last_observed_at: isNumber,
  last_source: isString,
  last_classification: isString,
  last_output_tps: isNumber,
  last_output_tokens: isNumber,
...
const eventValidator = hasShape({
  ...
  node_name: isString,
  reason: isString,
  classification: isString,
  output_tps: isNumber,
});
const configValidator = hasShape({
  mode: isOneOf("active", "passive", "hybrid"),
  model: isString,
  node_ids: isArrayOf(isString),
  active_interval_seconds: isNumber,
  passive_poll_seconds: isNumber,
  soft_tps: isNumber,
  hard_tps: isNumber,
  consecutive_soft: isNumber,
  consecutive_errors: isNumber,
  quarantine_seconds: isNumber,
  min_healthy_nodes: isNumber,
  max_output_tokens: isNumber,
  fail_closed: isBoolean,
  min_generation_ms: isNumber,
});
...
const decodeStatus = (value: unknown): QualityGuardStatus => {
  if (
    hasShape({ available: isBoolean })(value) &&
    (value as QualityGuardStatus).available === false
  ) {
    return value as QualityGuardStatus;
  }
  return createObjectDecoder<QualityGuardStatus>("quality guard", {
    available: isBoolean,
    editable: isOptional(isBoolean),
    startedAt: isNumber,
    updatedAt: isNumber,
    ...
    recentEvents: isArrayOf(eventValidator),
    statistics: isOptional(statisticsValidator),
  })(value);
};
...
export function getQualityGuardStatus(client: ApiClient): Promise<QualityGuardStatus> {
  return client.request("/api/admin/v1/egress-quality-guard", {}, decodeStatus);
}

export function runQualityTest(
  client: ApiClient,
  nodeId: string,
...
export function updateQualityGuardPolicy(
  client: ApiClient,
  policy: QualityGuardPolicy,
): Promise<{ saved: boolean }> {
  return client.request(
    "/api/admin/v1/egress-quality-guard/config",
    { method: "PUT", body: policy },
    createObjectDecoder("quality guard config update", { saved: isBoolean }),
  );
}
...
const decodeDegradeSummary = createObjectDecoder<DegradeSummaryDTO>("degrade accounts", {
  ...
  thresholds: hasShape({
    ...
    hardTPS: isNumber,
    minGenMs: isNumber,
    minOutputTokens: isNumber,
  }),
  totals: hasShape({
    hits: isNumber,
    accounts: isNumber,
    stillEnabled: isNumber,
    disabled: isNumber,
    deleted: isNumber,
    hard: isNumber,
    soft: isNumber,
  ...
  nodes: isArrayOf(
    hasShape({ name: isString, hits: isNumber, accounts: isNumber, maxTPS: isNumber }),
  ),
  accounts: isArrayOf(
    hasShape({
      id: isString,
...
export function getDegradeAccounts(
  client: ApiClient,
  input: {
    window: DegradeWindow;
    softTPS?: number;
  ...
  return client.request(
    `/api/admin/v1/request-audits/degrade-accounts?${query}`,
    {},
    decodeDegradeSummary,
  );
}
