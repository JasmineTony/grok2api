        ...
        <>
                ...
                {selectedNodes.length > 0 ? (
                  <>
                    ...
                    <Button
                      ...
                      size="sm"
                      disabled={
                        batchToggleMutation.isPending || selectedNodes.every((node) => node.enabled)
                      }
                      onClick={() =>
          ...
          >
            <AlertDialogContent>
              <AlertDialogHeader>
                <AlertDialogTitle>
                  {deletingNodes.length > 1
                    ? t("qualityGuard.deleteNodesTitle", { count: deletingNodes.length })
                    : t("qualityGuard.deleteNodeTitle")}
                </AlertDialogTitle>
                <AlertDialogDescription>
                  {deletingNodes.length > 1
                    ? t("qualityGuard.deleteNodesDescription", { count: deletingNodes.length })
                    : t("qualityGuard.deleteNodeDescription", {
                        name: deletingNodes[0]?.name ?? "",
                      })}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogCancel disabled={deleteNodeMutation.isPending}>
                  {t("common.cancel")}
                </AlertDialogCancel>
                <AlertDialogAction
                  className="bg-destructive text-white hover:bg-destructive/90"
                  disabled={deleteNodeMutation.isPending || deletingNodes.length === 0}
                  onClick={(event) => {
                    event.preventDefault();
                    if (deletingNodes.length > 0) deleteNodeMutation.mutate(deletingNodes);
                  }}
                >
                  {deleteNodeMutation.isPending ? <Spinner /> : null}
                  {t("common.delete")}
                </AlertDialogAction>
              </AlertDialogFooter>
            </AlertDialogContent>
