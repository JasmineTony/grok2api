...
export function DegradeAccountsPanel({
  ...
  const query = useQuery({
    ...
    queryFn: () =>
      getDegradeAccounts(apiClient, {
        window: period,
        ...(softTPS === undefined ? {} : { softTPS }),
        ...(hardTPS === undefined ? {} : { hardTPS }),
        ...(failClosed === undefined ? {} : { failClosed }),
        ...(minGenMs === undefined ? {} : { minGenMs }),
  ...
  const selectable = useMemo(
    () => rows.filter((account) => account.found && account.enabled),
    [rows],
  );
  ...
  const muteMutation = useMutation({
    mutationFn: (ids: string[]) => updateAccountsEnabledAction(apiClient, ids, false, "grok_build"),
    ...
    <div className="space-y-4">
      ...
      >
        <Metric
          ...
          value={String(data.totals.hits)}
          detail={t("qualityGuard.degrade.hitsHelp", {
            burst: data.totals.burst,
            soft: data.totals.soft,
            hard: data.totals.hard,
          })}
          tone={data.totals.hits ? "bad" : "good"}
        />
        <Metric
          icon={Users}
          label={t("qualityGuard.degrade.accounts")}
          value={String(data.totals.accounts)}
          detail={t("qualityGuard.degrade.accountsHelp", { deleted: data.totals.deleted })}
        />
        <Metric
          icon={PowerOff}
          label={t("qualityGuard.degrade.stillEnabled")}
          value={String(data.totals.stillEnabled)}
          detail={t("qualityGuard.degrade.stillEnabledHelp")}
          tone={data.totals.stillEnabled ? "bad" : "good"}
        />
        <Metric
          icon={Gauge}
          label={t("qualityGuard.degrade.maxTPS")}
          value={Math.round(data.totals.maxTPS).toString()}
          detail={t("qualityGuard.degrade.maxTPSHelp")}
          {...(data.totals.maxTPS >= data.thresholds.hardTPS ? { tone: "bad" as const } : {})}
        />
      </section>
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            <FilterSelect
              ...
              onChange={(value) => {
                setSelected(new Set());
                setPage(1);
                setPeriod(value as DegradeWindow);
              }}
              items={[
            ...
            <FilterSelect
              value={status}
              onChange={(value) => {
                setStatus(value as "all" | "enabled" | "disabled" | "deleted");
                setPage(1);
                setSelected(new Set());
              }}
              items={[
            ...
            <FilterSelect
              value={cls}
              onChange={(value) => {
                setCls(value as "all" | DegradeClass);
                setPage(1);
                setSelected(new Set());
              }}
              items={[
            ...
            />
            <FilterSelect
              value={String(hitsMin)}
              onChange={(value) => {
                setHitsMin(Number(value));
                setPage(1);
                setSelected(new Set());
              }}
              items={[
            ...
            >
              <PowerOff />
              {selectedRows.length
                ? t("qualityGuard.degrade.muteSelectedCount", { count: selectedRows.length })
                    ...
                    >
                      ...
                      <TableCell className="whitespace-nowrap">
                        ...
                        {account.bfs ? (
                          <Badge
                            variant="outline"
                            className="ml-1 whitespace-nowrap text-muted-foreground"
                          >
                            bfs {account.bfs}
                          </Badge>
                        ) : null}
                      ...
                      <TableCell
                        className={cn(
                          ...
                          account.maxTPS >= data.thresholds.hardTPS
                            ? "text-destructive"
                            : account.maxTPS >= data.thresholds.softTPS
                      ...
                      <TableCell className="text-xs text-muted-foreground">
                        {classSummary(account.classes)}
                      </TableCell>
                      <TableCell className="text-xs">{account.nodes.join(" · ") || "-"}</TableCell>
                      <TableCell className="font-mono text-xs">
                        {account.last ? formatDateTime(account.last, i18n.language) : "-"}
                      </TableCell>
                    </TableRow>
                  );
                })
      ...
      <section className="overflow-hidden rounded-lg bg-card">
        <div className="border-b px-4 py-4 sm:px-5">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            {t("qualityGuard.degrade.eventsHelp")}
          </p>
        </div>
            ...
            data.events.map((event) => (
              ...
              >
                ...
                <div className="truncate text-xs">
                  #{event.accountId ?? "-"} {event.accountName} · {event.nodeName} · out{" "}
                  {event.outputTokens} · {event.requestId}
                </div>
      ...
      <AlertDialog open={muteConfirmOpen} onOpenChange={setMuteConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("qualityGuard.degrade.muteSelected")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("qualityGuard.degrade.muteConfirm", { count: selectedRows.length })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={(event) => {
                ...
                muteMutation.mutate(
                  selectedRows.map((account) => account.id),
                  {
                    onSuccess: () => setMuteConfirmOpen(false),
                  },
                );
              }}
            >
                ...
                <div className="relative min-h-0 flex-1">
                  <div
                    className={cn(
                      ...
                      item.severe > 0 && item.severe >= item.count * 0.5
                        ? "bg-destructive"
                        : "bg-amber-500",
                    )}
                    style={{ height: `${height}%` }}
...
function shortSeriesLabel(label: string) {
  ...
  return hour?.[1] ? hour[1].padStart(2, "0") : label;
}
