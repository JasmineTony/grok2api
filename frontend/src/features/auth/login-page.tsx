      ...
      <main className="mx-auto flex w-full max-w-[960px] flex-1 items-center justify-center px-5 py-12 sm:px-8 lg:px-0">
        <div className="grid w-full max-w-[840px] items-center lg:grid-cols-[minmax(0,1fr)_1px_360px] lg:gap-14">
          <section className="hidden min-h-72 flex-col justify-center lg:flex">
            ...
            <h2 className="mt-3 max-w-sm text-3xl font-semibold leading-tight tracking-tight">{t("auth.productTitle")}</h2>
            <p className="mt-4 max-w-sm text-sm leading-6 text-muted-foreground">{t("auth.subtitle")}</p>
          </section>
          ...
          <section className="w-full max-w-[360px] justify-self-center rounded-xl border bg-card p-6 shadow-sm lg:justify-self-auto">
            <div className="mb-6"><h1 className="text-xl font-semibold">{t("auth.title")}</h1><p className="mt-2 text-xs leading-5 text-muted-foreground lg:hidden">{t("auth.subtitle")}</p></div>
            <form className="space-y-4" onSubmit={form.handleSubmit(submit, invalid)} noValidate>
              <div className="space-y-2"><Label htmlFor="username">{t("auth.username")}</Label><Input id="username" className="h-9 bg-background" autoComplete="username" autoFocus aria-invalid={Boolean(form.formState.errors.username)} aria-describedby={form.formState.errors.username ? "username-error" : undefined} {...form.register("username")} />{form.formState.errors.username ? <p id="username-error" role="alert" className="text-xs text-destructive">{form.formState.errors.username.message}</p> : null}</div>
              <div className="space-y-2"><Label htmlFor="password">{t("auth.password")}</Label><Input id="password" className="h-9 bg-background" type="password" autoComplete="current-password" aria-invalid={Boolean(form.formState.errors.password)} aria-describedby={form.formState.errors.password ? "password-error" : undefined} {...form.register("password")} />{form.formState.errors.password ? <p id="password-error" role="alert" className="text-xs text-destructive">{form.formState.errors.password.message}</p> : null}</div>
              <Button type="submit" size="sm" className="w-full" disabled={form.formState.isSubmitting}>{form.formState.isSubmitting ? t("auth.signingIn") : t("auth.signIn")}</Button>
            </form>
          </section>
        </div>
      </main>
