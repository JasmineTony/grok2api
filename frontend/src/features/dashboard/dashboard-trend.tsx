...
export function DashboardTrend({ dashboard, locale, loading }: DashboardTrendProps) {
  ...
  const xTicks = useMemo(
    () =>
      chartData
        .filter((_point, index) =>
          shouldShowTick(index, chartData.length, dashboard?.period ?? "24h"),
        )
  ...
  const hasData =
    dashboard?.series.some(
      (bucket) => bucket.requests > 0 || bucket.tokens > 0 || bucket.billedCostUsdTicks > 0,
    ) ?? false;
          ...
          <ChartContainer config={chartConfig} className="h-[280px] w-full aspect-auto">
            ...
            >
              ...
              <XAxis
                ...
                minTickGap={12}
                tickFormatter={(value) =>
                  formatBucketTick(String(value), dashboard?.period ?? "24h", locale)
                }
              />
              <YAxis
                yAxisId="tokens"
                      ...
                      <div className="flex w-full items-center justify-between gap-4">
                        <span className="flex min-w-0 items-center gap-2 text-xs font-normal text-muted-foreground">
                          <span
                            ...
                            style={{
                              backgroundColor: item.color || `var(--color-${String(name)})`,
                            }}
                          />
    ...
    <div className="flex flex-wrap items-center justify-center gap-x-4 gap-y-2 pt-3 text-xs text-muted-foreground">
      {TREND_SERIES.map((series) => {
        ...
        const accessibleLabel =
          typeof label === "string" || typeof label === "number" ? String(label) : series;
        return (
          <button
            key={series}
            type="button"
  ...
  if (series === "billing") {
    return (
      <span
        ...
        style={{ backgroundColor: "var(--color-billing)" }}
      />
    );
  }
  return (
...
): Partial<Record<TrendSeries, AxisSide>> {
