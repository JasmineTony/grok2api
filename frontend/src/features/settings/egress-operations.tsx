...
async function testAllEgressNodes(client: ReturnType<typeof useApiClient>) {
  ...
  for (let index = 0; index < ids.length; index += egressProbeBatchSize) {
    const batch = await testEgressNodes(client, ids.slice(index, index + egressProbeBatchSize));
...
export function EgressOperations({ scopeLabel }: { scopeLabel: (scope: EgressScope) => string }) {
  ...
  const [operationsDraft, setOperationsDraft] = useState<Omit<
    ...
    <section className="space-y-8">
      <div className="space-y-3">
        ...
        {operationsQuery.isError ? (
          <ErrorState
            message={operationsQuery.error.message}
          ...
          <div className="space-y-0">
            ...
            >
              <IntervalInput
                ...
                value={operationsForm.probeIntervalSeconds}
                unit={t("settings.units.seconds")}
                onChange={(probeIntervalSeconds) =>
                  setOperationsDraft({ ...operationsForm, probeIntervalSeconds })
                }
              />
            </AutomationRow>
            <AutomationRow
            ...
            >
              <IntervalInput
                ...
                value={operationsForm.assignmentIntervalSeconds}
                unit={t("settings.units.seconds")}
                onChange={(assignmentIntervalSeconds) =>
                  setOperationsDraft({ ...operationsForm, assignmentIntervalSeconds })
                }
              />
            </AutomationRow>
            <AutomationRow
            ...
            >
              <div className="flex h-8 items-center">
                <Switch
                  ...
                  onCheckedChange={(autoAssignEnabled) =>
                    setOperationsDraft({ ...operationsForm, autoAssignEnabled })
                  }
                />
              </div>
            </AutomationRow>
            ...
            >
              <div className="flex h-8 items-center">
                <Switch
                  ...
                  onCheckedChange={(autoBalanceEnabled) =>
                    setOperationsDraft({ ...operationsForm, autoBalanceEnabled })
                  }
                />
              </div>
            </AutomationRow>
                    ...
                    >
                      ...
                      >
                        <Select
                          value={fallback.mode}
                          onValueChange={(mode) =>
                            setFallbackMode(scope, mode as EgressFallbackMode)
                          }
                        >
                          <SelectTrigger
                          ...
                          <SelectContent>
                            <SelectItem value="none">
                              {t("settings.egress.fallbackNone")}
                            </SelectItem>
                        ...
                        {fallback.mode === "fixed" ? (
                          <Select
                            ...
                            onValueChange={(nodeId) =>
                              setFallback(scope, { mode: "fixed", nodeId })
                            }
                          >
                            <SelectTrigger
      ...
      <div className="space-y-3">
        ...
        >
          ...
          <ActionTooltip label={t("settings.egress.addSourceHelp")}>
            <Button type="button" size="sm" variant="secondary" onClick={() => openSource()}>
              <Plus />
        ...
        {sourcesQuery.isError ? (
          ...
          <div className="overflow-hidden rounded-md border">
            <Table className="min-w-[640px]">
              <TableHeader>
                <TableRow>
                  <TableHead className="min-w-48">{t("settings.egress.source")}</TableHead>
                  <TableHead className="w-32 text-center">{t("settings.egress.scope")}</TableHead>
                  <TableHead className="min-w-44">{t("settings.egress.lastSync")}</TableHead>
                  <TableHead className="w-28 text-center">
                    {t("settings.egress.capacity")}
                  </TableHead>
                  <TableActionHead />
                </TableRow>
              </TableHeader>
              <TableBody>
                ...
                {!sourcesQuery.isPending && (sourcesQuery.data?.items.length ?? 0) === 0 ? (
                  <TableRow>
                    <TableCell
                      colSpan={5}
        ...
        <DialogContent className="max-h-[calc(100svh-2rem)] overflow-y-auto sm:max-w-[520px]">
          ...
          >
            <ToggleControl
              ...
              onChange={(enabled) => setSourceForm({ ...sourceForm, enabled })}
            />
            ...
            <Control label={t("settings.egress.scope")}>
              <ScopeSelect
                ...
                onChange={(scope) => setSourceForm({ ...sourceForm, scope })}
            ...
            <Control label={t("settings.egress.subscriptionURL")}>
              <Input
                ...
                placeholder={
                  sourceEditing?.urlConfigured ? t("settings.egress.keepConfigured") : "https://..."
                }
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <Control label={t("settings.egress.refreshInterval")}>
                <Input
                  ...
                  min={60}
                  max={86400}
                  value={sourceForm.refreshIntervalSeconds}
                  onChange={(event) =>
                    setSourceForm({
                      ...sourceForm,
              ...
              <Control label={t("settings.egress.capacity")}>
                <Input
                  ...
                  value={sourceForm.defaultAccountCapacity || ""}
                  onChange={(event) =>
                    setSourceForm({
                      ...sourceForm,
                      defaultAccountCapacity: Number(event.target.value),
                    })
                  }
                />
              </Control>
          ...
          >
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <Control label={t("settings.egress.scope")}>
                <ScopeSelect
                  ...
                  onChange={(scope) => setImportForm({ ...importForm, scope })}
                  scopeLabel={scopeLabel}
                />
              </Control>
            </div>
            <Control label={t("settings.egress.capacity")}>
              <Input
                ...
                max={100000}
                placeholder={t("settings.egress.unlimited")}
                value={importForm.accountCapacity || ""}
                onChange={(event) =>
                  setImportForm({ ...importForm, accountCapacity: Number(event.target.value) })
                }
              />
            </Control>
            <Control label={t("settings.egress.proxyList")}>
              <Textarea
                ...
                onChange={(event) => setImportForm({ ...importForm, content: event.target.value })}
              />
            </Control>
            <DialogFooter>
              <Button
...
function OperationSectionHeader({
  title,
  help,
  children,
}: {
