...
const decodeModelAccounts = createObjectDecoder<{ items: ModelAccountOptionDTO[] }>(
  "model accounts",
  { items: isArrayOf(modelAccountValidator) },
);

export function listModels(
  client: ApiClient,
  input: ListModelsInput,
): Promise<PaginatedDTO<ModelRouteDTO>> {
...
const decodeModelSyncEvent = createObjectDecoder<ModelSyncEventDTO>("model sync event", {
  ...
  partial: isOptional(isBoolean),
  accountsSucceeded: isOptional(isNumber),
  accountsFailed: isOptional(isNumber),
...
export type ModelSyncResultDTO = {
  synced: number;
  partial: boolean;
  accountsSucceeded: number;
  accountsFailed: number;
...
export async function syncModels(
  client: ApiClient,
  onProgress?: (progress: ModelSyncProgressDTO) => void,
): Promise<ModelSyncResultDTO> {
  let result: ModelSyncResultDTO | undefined;
  await client.eventStream(
    "/api/admin/v1/models/sync",
    { method: "POST", headers: { Accept: "text/event-stream" } },
    decodeModelSyncEvent,
    ({ event, data }) => {
      if (
        event === "progress" &&
        ...
        onProgress?.({
          completed: Math.min(Math.max(0, data.completed), data.total),
          total: data.total,
        });
        return;
      }
      if (event === "complete" && typeof data.synced === "number") {
        result = {
          ...
          accountsSucceeded: data.accountsSucceeded ?? 0,
          accountsFailed: data.accountsFailed ?? 0,
        };
        return;
      }
      if (event === "error") {
        const code = data.code ?? "modelSyncFailed";
        const message = i18n.exists(`apiErrors.${code}`)
...
export function listModelAccountOptions(
  client: ApiClient,
  provider: ModelRouteDTO["provider"],
): Promise<{ items: ModelAccountOptionDTO[] }> {
  return client.request(
    `/api/admin/v1/models/accounts?provider=${provider}`,
    {},
    decodeModelAccounts,
  );
}

export function createModel(client: ApiClient, input: CreateModelInput): Promise<ModelRouteDTO> {
  return client.request("/api/admin/v1/models", { method: "POST", body: input }, decodeModelRoute);
}

export function updateModel(
  client: ApiClient,
  id: string,
  input: { publicId: string; enabled: boolean; accountIds: string[] },
): Promise<ModelRouteDTO> {
  return client.request(
    `/api/admin/v1/models/${id}`,
    { method: "PATCH", body: input },
    decodeModelRoute,
  );
}

export function deleteModel(client: ApiClient, id: string): Promise<{ deleted: boolean }> {
  return client.request(
    `/api/admin/v1/models/${id}`,
    { method: "DELETE" },
...
export function deleteModels(client: ApiClient, ids: string[]): Promise<{ deleted: number }> {
  return client.request(
    "/api/admin/v1/models",
...
export function updateModelsEnabled(
  client: ApiClient,
  ids: string[],
  enabled: boolean,
): Promise<{ updated: number }> {
  return client.request(
    "/api/admin/v1/models/batch",
