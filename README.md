...
当前源码版本为 **v3.2.0**，已依次合并上游 `v3.0.8-hotfix.1` 与 `v3.0.9`，并保留独立仓库的可靠性治理、Egress、设置页面组件化和兼容性约束。公开 API、配置语义、数据库兼容性和 Go module 路径保持不变。
...
稳定镜像位于 `ghcr.io/jasminetony/grok2api`。v3.2.0 发布完成后可使用 `v3.2.0`、`3.2.0`、`3.2`、`3` 或 `latest` 标签；发布完成前请继续固定到已经验证的 v3.1.1 digest。若部署环境无法访问 GHCR，仍可使用 `docker compose up -d --build` 从当前源码构建。
