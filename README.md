...
当前源码版本为 **v3.3.0**，已精确同步上游 `v3.0.10`，并保留独立仓库的可靠性治理、Egress、完整设置契约、五路由设置架构、Lucide 图标规范和兼容性约束。公开 API、配置语义、数据库兼容性和 Go module 路径保持不变。
...
稳定镜像位于 `ghcr.io/jasminetony/grok2api`。v3.3.0 发布完成后可使用 `v3.3.0`、`3.3.0`、`3.3`、`3` 或 `latest` 标签；发布完成前请继续固定到已经验证的 v3.2.0 digest。若部署环境无法访问 GHCR，仍可使用 `docker compose up -d --build` 从当前源码构建。
